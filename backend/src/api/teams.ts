...
export const teams = (db: Surreal) => new Elysia({ prefix: "/teams", tags: ["Teams"] })
...
.post("/:id/members", async ({ params: { id }, body: { user } }) => {
...
.patch("/:tid/members/:uid", ({ params: { tid , uid} }) => {

}, { body: tMember, detail: { description: "Modifies a member of a team" } })

.delete("/:tid/members/:uid", async ({ params: { tid , uid} }) => {
